...
//        stage_color(&tlc_colors_next[0], (tlc_anim_index+1) % tlc_curr_anim_len);
        ...
        led_ring_dest[0].r = led_ring_anim_curr.colors[(led_ring_anim_index+1) % led_ring_anim_curr.len].r;
        led_ring_dest[0].g = led_ring_anim_curr.colors[(led_ring_anim_index+1) % led_ring_anim_curr.len].g;
        led_ring_dest[0].b = led_ring_anim_curr.colors[(led_ring_anim_index+1) % led_ring_anim_curr.len].b;

        led_ring_step[0].r = ((int_fast16_t) led_ring_dest[0].r - led_ring_curr[0].r) / led_ring_anim_curr.speed;
        led_ring_step[0].g = ((int_fast16_t) led_ring_dest[0].g - led_ring_curr[0].g) / led_ring_anim_curr.speed;
        led_ring_step[0].b = ((int_fast16_t) led_ring_dest[0].b - led_ring_curr[0].b) / led_ring_anim_curr.speed;
    }
}

/// Send the colors to the underlying controller.
void led_set_colors() {
    led_all_one_color_ring_only(led_ring_curr[0].r,
                                led_ring_curr[0].g,
                                led_ring_curr[0].b);
}
...
void led_set_anim(led_ring_animation_t anim, uint8_t anim_type) {
    ...
    led_ring_curr[0].r = anim.colors[0].r;
    led_ring_curr[0].g = anim.colors[0].g;
    led_ring_curr[0].b = anim.colors[0].b;
